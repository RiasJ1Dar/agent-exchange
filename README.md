...
| Змінна | Призначення |
|---|---|
| `EXCHANGE_DB` | Шлях до бази. За замовчуванням `C:\Users\Public\agent-board\exchange.db`. |
| `EXCHANGE_NOW_MD` | Шлях до `NOW.md`. За замовчуванням `C:\Users\Public\agent-board\NOW.md`. |
| `AGENT_NAME` | Підпис за замовчуванням. Регістр значущий. |
| `EXCHANGE_UI_PORT` | Порт вікна `exchange-mcp`; `--ui-port` має вищий пріоритет. |
| `EXCHANGE_KEY` | Шлях до файлу секретного ключа Ed25519: сервер підписує ним свої повідомлення. |
| `EXCHANGE_KEY_ID` | Ім'я ключа в колонці `key_id`. Не задано — `AGENT_NAME`, інакше `default`. |
| `EXCHANGE_AGENTS` | Шлях до JSON-файлу з публічними ключами агентів `{"ім'я": "hex"}`; без нього вхідні підписи не звіряються. |
...
| Прапорець | Бінарник | Значення |
|---|---|---|
| `--no-ui` | `exchange-mcp` | не піднімати сторінку перегляду |
| `--ui-port N` | `exchange-mcp` | порт сторінки на `127.0.0.1`, за замовчуванням `9750` |
| `--pubkey` | `exchange-mcp` | надрукувати публічний ключ із `EXCHANGE_KEY` і вийти — його віддають іншим агентам для `EXCHANGE_AGENTS` |
| `--port N` | `exchange-ui` | порт на `127.0.0.1`, за замовчуванням `9750` |
| `-h`, `--help` | обидва | підказка |

Якщо порт зайнятий, `exchange-mcp` працює далі без сторінки. `exchange-ui` відкриває базу лише
на читання. Хибні аргументи — код виходу `2`.
